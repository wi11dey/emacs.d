...
(p@ck ivy
  ...
  (p@ck swiper
    ...
    (p@ck my/$-mc-mark
      ...
      (defvar $-start nil)
      !(defun $-finish (_cand)
	 (setq ivy-inhibit-action nil))
      !(defun $-unwind ()
	 (swiper--cleanup)
	 (setf (ivy-state-unwind ivy-last) #'swiper--cleanup)
	 (if (eq ivy-exit 'done)
	     (let ((overlays (overlays-in (point-min) (point-max)))
		   overlay)
	       (while (setq overlay (pop overlays))
		 (when (eq (overlay-get overlay 'type) 'additional-region)
		   (overlay-put overlay 'face 'mc/region-face)))
	       (when $-start
		 (setq ivy-inhibit-action #'$-finish)
		 (mc/pop-state-from-overlay $-start)
		 (multiple-cursors-mode 1)))
	   (multiple-cursors-mode -1))
	 (setq $-start nil))
      !(defun $-action (cand &optional action always)
	 (let ((mark-active t)
	       cursor
	       region
	       fake-cursors)
	   (funcall (or action
		    ...
		    cand)
	   (re-search-forward ivy--old-re nil t (if swiper-goto-start-of-match
						    1
						  -1))
	   (if (and (not always)
		    (setq fake-cursors (mc/all-fake-cursors (match-beginning 0)
							    (match-end       0))))
	       (progn
		 (dolist (overlay fake-cursors)
		   (mc/remove-fake-cursor overlay))
	     ...
	     (setq cursor (mc/create-fake-cursor-at-point))
	     (unless $-start
	       (message "BBB")
	       (setq $-start cursor))
	     (when (setq region (overlay-get cursor 'region-overlay))
	       (overlay-put region 'face nil)))
	   (setf (ivy-state-unwind ivy-last) #'$-unwind)))
      ;;;;;; All
      !(defun $-all ()
	 (interactive)
	 (let ((cands (nreverse ivy--old-cands))
	   ...
	   (unless (string= ivy-text "")
	     (ivy-exit-with-action
	      (lambda (_)
		(let (cand)
		  ($-action cand action :always)
		  (while (setq cand (pop cands))
		    ($-action cand action :always))))))))
      (add-hook 'mc/cmds-to-run-once #'$-all)
      ;;;;;; Next
      !(defun $ ()
	 (interactive)
	 (unless $-start
	   (let ((ivy-inhibit-action #'$-action))
	     (ivy-call)))
	 (ivy-next-line)
	 (ivy--exhibit)
	 (let ((ivy-inhibit-action #'$-action))
	   (ivy-call)))
      (add-hook 'mc/cmds-to-run-once #'$))
    _(bind-key "C->" #'my/$-mc-mark $-map)
    _(bind-key "C-!" #'my/$-mc-mark-all $-map)
